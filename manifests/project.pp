...
define typo3::project (
  ...
  exec { "ln -s typo3_src/index.php index.php":
    command => '/bin/ln -s typo3_src/index.php index.php',
    cwd     => $site_path,
    require => File["${site_path}/typo3_src"],
    unless  => '/usr/bin/test -L index.php',
  }

  unless $version =~ /^6\.2/ {
    exec { "/bin/ln -s typo3_src/t3lib t3lib":
      command   => '/bin/ln -s typo3_src/t3lib t3lib',
      cwd       => $site_path,
      require   => File["${site_path}/typo3_src"],
      unless    => '/usr/bin/test -L t3lib',
    }
  }

  exec { "/bin/ln -s typo3_src/typo3 typo3":
    command => '/bin/ln -s typo3_src/typo3 typo3',
    cwd     => $site_path,
    require => File["${site_path}/typo3_src"],
    unless  => '/usr/bin/test -L typo3',
  }
